...
var github_repository      = 'https://rawgit.com/Snipeon/mirrors/master/';
...
var localMirrors = [];
var githubMirrors = ["MangaHere", "MangaFox", "Manga Reader"];
...
{
	"use strict";
	var re = /^s?https?:\/\/[-_.!~*'()a-zA-Z0-9;\/?:\@&=+\$,%#]+$/, res = str.match(re);
...
function getMirrorsDescription(callback)
{
	...
	{
		var websites = list, mustUpdate = false, i;
...
function loadJSFromRepository(description)
{
	...
	amrcsql.webdb.storeWebsite(description, function(){description.loaded = true;});
}
...
function waitForFinishRepository(websites, callback)
{
	...
	{
		setTimeout(function(){waitForFinishRepository(websites, callback);}, 100);
	}
}
...
function updateWebsitesFromRepository(callback)
{
	...
	({
		type: 'GET',
		url: amrc_repository + "websites.json",
		error: function(xhr, status, error){amrc_repository = amrc_repository_backup;},
	});
		...
		success : function(resp)
		{
			...
			amrcsql.webdb.getWebsites(function(list)
			{
				...
				if(wsloc !== undefined && wsloc !== null && wsloc.length > 0)
				{
					...
					if(changes.length > 0)
					{
						waitForFinishUpdatingRepository(changes, function(){callback();});
					}
					else
					{
						callback();
...
function updateJSFromRepository(description, change)
{
	...
	if(change.type === "create")
	{
		...
		amrcsql.webdb.storeWebsite(description, function(){change.loaded = true;});
		isNew = true;
	}
	else if(change.type === "update")
	{
		console.log("update " + description.mirrorName + " in database");
		amrcsql.webdb.updateWebsite(description, function(){change.loaded = true;});
	}
...
function waitForFinishUpdatingRepository(changes, callback)
{
	...
	{
		setTimeout(function(){waitForFinishUpdatingRepository(changes, callback);}, 100);
	}
}
			...
			getMirrorsDescription(function(list)
			{
				...
				if(wsloc !== undefined && wsloc !== null && wsloc.length > 0)
				{
					...
					if(found === null)
					{
						...
						console.log("insert " + description.mirrorName + " in database");
						isNew = true;
						amrcsql.webdb.storeWebsite(description, function(){finishImportAfterInsert(description, callback, isNew);});
					} 
					else
					{
						//Mise ? jour de l'implem
						console.log("update " + description.mirrorName + " in database");
						amrcsql.webdb.updateWebsite(description, function(){finishImportAfterInsert(description, callback, isNew);});
					}
				}
				else 
				{
...
function getMirrors(callback)
{
	...
	getMirrorsDescription(function(list)
	{
		...
		chrome.extension.getBackgroundPage().isReady(true);
		waitForFinishgetMirrors(mirrorsTmp, function(){callback(mirrorsTmp);});
	});
}
...
function waitForFinishgetMirrors(mirrors, callback)
{
	...
	{
		setTimeout(function(){waitForFinishgetMirrors(mirrors, callback);}, 100);
	}
}
...
function doesCurrentPageMatchManga(url, activatedMirrors, callback)
{
	...
	getMirrorsDescription(function(list)
	{
		...
		for(i = 0; i < wsloc.length; i += 1)
		{
			...
			if(isFound)
			{
				var wss = (typeof wsloc[i].webSites === 'object') ? wsloc[i].webSites : JSON.parse(wsloc[i].webSites), j;
...
{
	"use strict";
	chrome.runtime.sendMessage({action : "activatedMirrors"}, function(res)
	{
...
function waitForActivatedAndListFinish(mirrorsT, callback)
{
	...
	{
		setTimeout(function(){waitForActivatedAndListFinish(mirrorsT, callback);}, 100);
	}
}
...
function displayNotification(wsData, params)
{
	...
	{
		setTimeout(function(){notif.cancel();}, params.notificationtimer * 1000);
	}
	*/
}
